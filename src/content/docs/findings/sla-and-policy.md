...
description: "Unlike enforcement mode's single inherited value, an SLA is a matrix; SlaRule rows keyed by (workspace_id, group_id | null, severity), each with a days_to_fix."
---
...
- **Risk Scoring** — the weights behind each finding's priority score; see [Priority scoring](/toleman/findings/lifecycle-and-scoring/#priority-scoring)
