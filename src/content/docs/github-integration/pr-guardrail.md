---
title: "PR Guardrail"
description: "PR Guardrail scans a pull request's diff for net-new findings only, and can block or warn on the PR via a GitHub commit status."
---
...
![PR History page](/toleman/img/screenshots/pr-history.png)
...
![Target detail: PR Guardrail enforcement dropdown](/toleman/img/screenshots/target-detail.png)

## Accept risk / ignore workflow

A developer can request an ignore on a specific finding (`POST /api/pr-guardrail/findings/{id}/request-ignore`), from a PR comment or the PR's scan detail. Every pending request across the workspace lands in one place for review, the **Approval Queue** page (`GET /api/pr-guardrail/ignore-requests/pending`). ![Approval Queue: pending ignore requests](/toleman/img/screenshots/approval-queue.png)
