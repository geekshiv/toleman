...
description: "Toleman can open a real PR against a target repo that adds a GitHub Actions workflow running Semgrep/Gitleaks/Trivy (+ gosec, if Go is detected) natively and pushing results back to Toleman."
---
...
Opening a PR that adds `.github/workflows/*` requires the GitHub App's separate **workflows** permission (distinct from `contents`). See [Connecting GitHub](/toleman/github-integration/connecting-github/) for what to do if you get a 403 here on an older App install.
