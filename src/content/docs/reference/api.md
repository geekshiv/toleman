...
description: "A versioned, token-authenticated API for scripts, CI, and third-party integrations; distinct from the internal API the frontend uses (session-cookie authenticated) and from the workspace API key (CI-ingest-only, /api/ingest/{target_id})."
---
...
![Settings: API Tokens card](/toleman/img/screenshots/settings-workspace.svg)
